...
module.exports.run = async (client, message, args, ecoPool, connection, stats) => {
	...
	connection.query(`UPDATE stats SET bank = '${stats.bank - args[0]}' WHERE userID = '${message.author.id}'`);
	connection.query(`UPDATE stats SET cash = '${stats.cash + args[0]}' WHERE userID = '${message.author.id}'`);

	message.channel.send('You have successfully withdrawn **' + args[0] + '** from your bank! \nYou have **' + stats.bank - args[0] + '** left inside your bank!');
};
