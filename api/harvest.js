...
module.exports = (database, stats) => {
	if(!stats.creation.time || Date.now() <= stats.creation.time) {
		...
		const harvested = harvestRewards[stats.creation.type].map(reward => {
			stats.stocks[reward[1]] = Number(stats.stocks[reward[1]] == undefined ? 0 : Number(stats.stocks[reward[1]])) + Number(Number(reward[0] * luck * stats.creation.amount).toFixed(0));
