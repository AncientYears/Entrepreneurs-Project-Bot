...
module.exports.run = async (client, message, ecoPool) => { // commandhandler.run
	...
	if (cmd) {
		...
		console.log(`[Ping:${Math.round(client.ws.ping)}ms] ${cmd.help.name} request by ${message.author.username} @ ${message.author.id} `); // if command can run => log action
		const stats = await client.zumzaApi.getStats(message.author.id, ecoPool).then(data => data.data);
		if (cmd.help.requires) {
			if (cmd.help.requires.includes('botowner')) if (!['193406800614129664', '211795109132369920'].includes(message.author.id)) return message.reply('This command cannot be used by you!'), console.log(`[Ping:${Math.round(client.ws.ping)}ms] ${cmd.help.name} failed!: Not Bot Owner! `), message.channel.stopTyping(true);
			if (cmd.help.requires.includes('guild') && message.channel.type !== 'text') return message.channel.send('This command needs to be run in a guild!'), console.log(`[Ping:${Math.round(client.ws.ping)}ms] ${cmd.help.name} failed!: Not Guild! `), message.channel.stopTyping(true);
			if (cmd.help.requires.includes('dm') && message.channel.type !== 'dm') return message.channel.send('This command needs to be run in DMs!'), console.log(`[Ping:${Math.round(client.ws.ping)}ms] ${cmd.help.name} failed!: Not DM! `), message.channel.stopTyping(true);
			if (cmd.help.requires.includes('business') && (!stats || !stats.business.location.length)) return message.channel.send(`Seems like you dont have a business yet! Create one by using **${client.prefix}setup**`), console.log(`[Ping:${Math.round(client.ws.ping)}ms] ${cmd.help.name} failed!: No Business! `), message.channel.stopTyping(true);
			if (cmd.help.requires.includes('farm') && (!stats || !stats.business || stats.business.type !== 'farm')) return message.channel.send('Sorry, you do not have a farm! \nYou have a **' + stats.business.type + '**'), console.log(`[Ping:${Math.round(client.ws.ping)}ms] ${cmd.help.name} failed!: No Farm! `), message.channel.stopTyping(true);
			if (cmd.help.requires.includes('shop') && (!stats || !stats.business || stats.business.type !== 'shop')) return message.channel.send('Sorry, you do not have a shop! \nYou have a **' + stats.business.type + '**'), console.log(`[Ping:${Math.round(client.ws.ping)}ms] ${cmd.help.name} failed!: No Shop! `), message.channel.stopTyping(true);
			if (cmd.help.requires.includes('factory') && (!stats || !stats.business || stats.business.type !== 'factory')) return message.channel.send('Sorry, you do not have a factory! \nYou have a **' + stats.business.type + '**'), console.log(`[Ping:${Math.round(client.ws.ping)}ms] ${cmd.help.name} failed!: No Factory! `), message.channel.stopTyping(true);
